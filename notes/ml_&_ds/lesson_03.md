...
2. Имеет смысл разные признаки брать с разными весами, то есть водить веса признаков $$w_i$$
...
Когда стоит задача классифицировать некий объект $$x \in X$$
 и есть объекты обучающей выборки $$x_1, ..., x_l$$
 , то самое простое, что можно сделать - это отранжировать объект обучающей выборки по возрастанию расстояний:
...
$$Г_y(x)$$ - оценка близости объекта **x** к классу **y**: сумма по всем объектам класса **y** (суммируем только по объектам
класса **y**)

$$w(i,x)$$ - вес (степень важности) i-го соседа объекта **x**, неотрицателен, не возрастает по i

Дальше веса или степени важности соседей суммируются по всем объектам данного класса **y**, и объект **x** причисляется к тому классу **y**, для которого суммарная оценка близости $$Г_y(x)$$ 
максимальна. То есть сколько соседей было в окрестности объекта **x** каждого из классов - какой класс оказался в большинстве в этой окрестности, значит тому классу и надо относить **x** 

Это непосредственная формализация **гипотезы компактности** и при этом используеся только **функция расстояния между объектами**


### [00:14:33 - Метод k ближайших соседей (k nearest neighbors, KNN)](https://youtu.be/GyOxB2itxnc?t=873)

$$w(i,x)$$ - это вес этого соседа для классификации объекта **x**

$$w(i,x)=[i\leqslant 1]$$ - метод ближайшего соседа

Только 1-ый сосед имеет вес $$w(i,x)=1$$ 
остальные все соседи просто не учитываются; то есть относи объект к тому классу, которому принадлежит его ближайший сосед

Это плохо тем, что случайно может по каким-то причинам объект одного класса попасть в сгусток объектов других классов тогда и он будет классифицироваться неправильно и соседний с ним будут классифицироваться неправильно

Надежность классификации можно повысить, если смотреть не на одного ближайшего соседа, а на несколько ближайших соседей. Причем возникает параметр - k ближайших соседей

$$w(i,x)=[i\leqslant k]$$ - метод k ближайших соседей 

Как выбрать этот самый параметр, сколько ближайших соседей?

Здесь на помощь приходит критерий **leave-one-out** - это критерий скользящего контроля:
1. Для каждого объекта $$x_i$$ 
берутся все объекты обучающей выборки кроме него самого
2. Для взятых объектов проводится оценка того, что правда ли, что по ближайшему окружению объекта $$x_i$$ 
действительно можно правильно классифицировать этот самый объект $$x_i$$


Преимущества:
- простота реализации (lazy learning): необходимо толко сохранить обучающую выборку
- параметр k можно оптимизировать по критерию скользящего контроля (leave-one-out): для вычисления LOO необходимо только исключить из обучающей выборки сам объект, который пытаемся классифицировать

$$LOO(k, X^l)=\sum_{i=1}^l [a(x_i, X^l\backslash \{x_i\}, k)\not=y_i] \rightarrow \min_k$$
...
### [00:17:08 - Зависимость LOO от числа соседей](https://youtu.be/GyOxB2itxnc?t=1028)

**Важно сам объект выкинуть из обучающей выборки, когда мы пытаемся на нем сделать классификацию**. Если этого не сделать, то результатом будет смещенная оптимистично заниженная оценка
числа ошибок, что можно назвать **переобучение**


### [00:19:33 - Метод k взвешанных ближайших соседей](https://youtu.be/GyOxB2itxnc?t=1173)

Можно немножечко **обобщить метод ближайших соседей** и сказать так: если 1-ый сосед должен иметь больший вес, чем 2-ой сосед, 2-ой больше, чем 3-ий. То есть кажется, что веса соседей разумно, чтобы убывали с ростом номера соседа: чем дальше объект расположен, тем меньше он должен влиять на классификацию данного соседа 

$$w(i,x)=[i\leqslant k]w_i$$

где $$w_i$$ 
- вес, зависящий только от номера соседа

**Как задать эти?**

**Возможные эвристики задания весов:**
1. Можно сделать веса линейно убывающими: $$w_i=(k+1-i)\backslash k$$. 
Это не очень хороший выбор. Например, когда 1-ый и 4-ый соседи лежат в одном классе, а 2-ой и 3-ий соседи лежат в другом классе -> тогда усредняя вот эти линейные веса для 2-х классов получатся одинаковые оценки и не получится с помощью **arg max** принять решение о том, какому же из этих 2-х классов отнести объект 
2. Чтобы таких случайных совпадений не происходило, веса нужно задать убывающими по какому-то не линейному закону, например, по экспоненте: $$w_i=q^i, 0<q<1$$.
Вот такой способ задания весов решает эту проблему

Возникает другой вопрос: допустим 3 первых соседа они близко лежат к объекту, а 4-ый сосед очень далеко, а используется методом 4-х ближайших соседей. Тогда почему вес объекта зависит от номера его в ранжированном списке, а не от расстояния? 
Было бы правильнее, если бы вводилась зависимость от расстояния до

Проблемы исползования взвешенных сосетедей по ранжированию:
- как более обоснованно задать веса?
- возможно, было бы лучше, если бы вес $$w(i,x)$$ 
зависел не от порядкового номера соседа **i**, а от расстояния до него $$\rho(x,x^{(i)})$$ 


### [00:21:32 - Метод окна Парзена](https://youtu.be/GyOxB2itxnc?t=1292)

Здесь весовой коэффициент $$w(i,x)$$ 
i-го соседа объекта **x** устроен следующим образом: это убывающая функция **K** от расстояния между **x** и его i-тым соседом $$\rho(x,x^{(i)})$$:

$$w(i,x)=K(\frac{\rho(x,x^{(i)})} {h})$$, где h - ширина окна

$$K(r)$$ - ядро, не возрастает, положительно на [0, 1]

Если подставить весовую функцию $$w(i,x)$$ 
в общую **конструкцию метрического классификатора**, то получим **метод парзенского окна**. Здесь фактически вместо параметра **k** ближайших соседей появляется другой параметр - это **ширина окна h**. Сколько объектов попадают в это окно -> по этим объектам принимается решение о том какому классу отнести **x**. Если, например, ядро обладает свойством, что оно больше 0 только на отрезке [0, 1], то фактически **h** просто показывает радиус окресности, в которой мы смотрим на точку. То есть все точки,которые вне радиуса **h** от центра **x** находятся - они как бы для нас не существуют

**Метод парзеновского окна фиксированной ширины:**

$$a(x; X^l, h, K)= \arg \max_{y \in Y} \sum_{i=1}^l [y_i=y] K(\frac{\rho(x,x_{(i)})} {h})$$

В чем опасность такого подхода? В том что объект **x** может оказаться далеко вообще от любых объектах обучающей выборки на расстояние $$>h$$ 
и тогда мы не сможем классифицировать этот объект

В этом случае есть два решения: 
1. Можно либо пользоваться только ядрами, которые имеют значение больше 0 от $$[0, +\infin]$$ 
2. Использовать **метод парзеновского окна переменной ширины:**

$$a(x; X^l, k, K)= \arg \max_{y \in Y} \sum_{i=1}^l [y_i=y] K(\frac{\rho(x,x_{(i)})} {\rho(x, x^{(k+1)})})$$

Во втором случаем мы говорим, что пусть **h** на самом деле определяется так, чтобы у нас в это окно всегда **k** соседей попадало. Для этого должны сделать взять расстояние для **k+1** соседа в качестве
